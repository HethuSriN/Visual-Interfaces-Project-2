...
function renderChart(data, viewType) {
    ...
    // Bars (the bars in the chart)
    const barSelection = chart.selectAll(".bar")
        .data(bins)
        .join("rect")
        .attr("class", "bar")
        .attr("x", d => xScale(`${d.x0} - ${d.x1}`))
    ...
    // Bar selection (this is the click event listener)
    barSelection.on("click", function(event, d) {
        const selectedRange = {
            min: d.x0,  // This is the minimum value from the selected bin
            max: d.x1   // This is the maximum value from the selected bin
        };
        ...
        const filteredData = combinedData.filter(item => {
            // Ensure the magnitude is a number before comparison
            const magnitude = +item.mag; // Convert to number explicitly
            const depth = +item.depth; // Convert to number explicitly
            const dmin = +item.dmin; // Convert to number explicitly
            console.log("Checking magnitude for data point:", magnitude); // Debug log
            if (depth) {
                console.log("Depth value:", depth); // Debug log for depth
                return depth >= selectedRange.min && depth <= selectedRange.max;
            }
            else if (magnitude) {
                return magnitude >= selectedRange.min && magnitude <= selectedRange.max;
            }
            else if(dmin){
                return dmin >= selectedRange.min && dmin <= selectedRange.max;
            }
            else{
                console.error("Invalid data point:", item); // Log invalid data points
                return false; // Exclude invalid data points
            }
        ...
        // Pass the filtered data to the map and heatmap
        renderFilteredMap(filteredData);  // Update the map
        renderFilteredHeatmap(filteredData, selectedBinGlobal);  // Update the heatmap
    });
...
// // Function to filter the data and update map, heatmap, and chart
// function filterAndUpdate(selectedRange) {
//     // Filter the data based on the selected range
//     const filteredData = combinedData.filter(d =>
//         d.magnitude >= selectedRange.min && d.magnitude < selectedRange.max
//     );

//     // Update the map, heatmap, and chart with filtered data
